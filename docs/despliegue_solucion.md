...
| **Application Pool** | shmappapi (No Managed Code) |
| **Puerto** | 92 |
...
| **Application Pool** | shmappwebadmin (No Managed Code) |
| **Puerto** | 91 |
...
| **Application Pool** | shmappwebcompania (No Managed Code) |
| **Puerto** | 93 |
| **Framework** | ASP.NET Core 8 MVC (out-of-process) |
...
**Permisos requeridos:**
- Application Pool `shmappwebcompania` → Lectura y Escritura
- Application Pool `shmappwebadmin` → Lectura
- Application Pool `shmappapi` → Sin acceso (no lo requiere)
...
| 4 | `04_create_view.sql` | Vistas de la base de datos |
| 5 | `05_insert_data_maestros.sql` | Datos maestros (tablas, bancos, etc.) |
| 6 | `06_insert_data.sql` | Datos iniciales del sistema |
| 7 | `07_insert_data_opcion_menu.sql` | Opciones del menu y permisos |

> **Nota:** Los scripts `99_*.sql` son datos de prueba y **NO deben ejecutarse en Produccion**.
...
|---------|------------------|----------------------|
| SHM_API | **No Managed Code** | Integrated |
| SHM_WebAdmin | **No Managed Code** | Integrated |
| SHM_WebCompania | **No Managed Code** | Integrated |
...
| Application pool | SHM_WebCompania |
...
2. **Agregar** > Escribir `IIS AppPool\SHM_API` > **Aceptar**
...
### 7.4 Sincronizacion inicial de datos

Ejecutar los siguientes endpoints del API para sincronizar datos maestros desde los sistemas externos:

```bash
# 1. Sincronizar sedes desde API San Pablo
curl -X POST https://[HOST_API]/api/SedeInterface/sync

# 2. Sincronizar bancos desde API SAP
curl -X POST https://[HOST_API]/api/SapInterface/sincronizar-bancos
```
...
[ ] 3. Base de datos creada (scripts 01 a 07 ejecutados)
...
[ ] 17. Sincronizacion de sedes ejecutada (POST /api/SedeInterface/sync)
[ ] 18. Sincronizacion de bancos ejecutada (POST /api/SapInterface/sincronizar-bancos)
[ ] 19. Password de SYSADMIN cambiado
[ ] 20. Logs se generan correctamente en carpeta Logs/
```
